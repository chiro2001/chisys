...
VNode *app() {
  ...
  VNode *root =
      (
chidom::h(u8"window", chidom::Data (chidom::Attrs {{u8"name", u8"test chiui"}, {u8"x", u8"20"}, {u8"y", u8"10"}, {u8"w", u8"320"}, {u8"h", u8"240"}}), chidom::Children {chidom::h(u8"button", chidom::Data (chidom::Callbacks {{u8"onclick", [](const Event &e) -> void {
    printf("OnClick\r\n");
  }}}), std::string(u8"Button Text")), chidom::h(u8"button", chidom::Data (chidom::Attrs {{u8"w", u8"160"}}), std::string(u8"Long Button!"))})
);
  return root;
}
